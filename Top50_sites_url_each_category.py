...
def crawl_all_links():
    ...
    for element in links:
        driver = webdriver.Chrome(executable_path=r"C:\ProgramData\chocolatey\lib-bad\chromedriver\tools\chromedriver.exe")
        url = f'https://www.alexa.com/topsites/category/Top/{str(element)}'
        driver.get(url)
        html = driver.page_source
        soup = BeautifulSoup(html)
        list_of_data = list(soup.find_all('div'))
...
with open("sites.csv") as f:
    ...
    sites_and_categories.extend(
        str("line{0} = {1}".format(i, x).split(' = [\'')[1].split('\']')[0])
        .replace(',', ' ')
        .split(' ')
...
for sites_and_category in sites_and_categories:
    print(sites_and_category[0])
    print(sites_and_category[1])
